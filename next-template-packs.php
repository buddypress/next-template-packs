...
class Next_Template_Packs {
	...
	private function setup_hooks() {
		...
		// BuddyPress is an unsupported version
		} else {
			add_action( $this->config['network_active']
				? 'network_admin_notices'
				: 'admin_notices', array( $this, 'admin_warning' ) );
		}
	...
	public function admin_menu() {
		$this->page = bp_core_do_network_admin()
			? 'settings.php'
			: 'options-general.php';
	...
	public function admin_load() {
		...
		// Bail if no template pack directory
	...
	public function admin_tab() {
		...
		$url = add_query_arg( array( 'page' => 'template-packs' ), bp_get_admin_url( 'admin.php' ) );

		?><a href="<?php echo esc_url( $url ); ?>" class="nav-tab <?php echo esc_attr( $class ); ?>"><?php esc_html_e( 'Template Packs', 'next-template-packs' );?></a><?php
	}
	...
	public static function check_versions( $versions = array() ) {
		$retval = true;
	...
	public function admin_display() {
		...
		$template_packs = self::get_template_packs( $tp_locations, $tp_urls );
		...
		<div class="wrap">
			...
			<form action="" method="post" id="bp-admin-template-pack-form">
				...
				<table class="widefat fixed plugins" cellspacing="0"><?php

					self::table_head();

					self::table_body( $template_packs );
	 ...
	 * Get template packs
	 *
	 * @param array $tp_locations
	 * @return \stdClass
	 */
	private static function get_template_packs( $tp_locations = array(), $tp_urls = array() ) {
		$template_packs = array();

		// Loop through locations and build base pack info
		...
		// Loop through packs, and translate headers
	 ...
	 */
	private static function table_head() {
		?>
	 ...
	 *
	 * @param array $template_packs
	 */
	private static function table_body( $template_packs = array() ) {
		...
		<tbody id="the-list"><?php
			...
			if ( ! empty( $template_packs ) ) :
				...
				foreach ( $template_packs as $tp_pack ) :
					$class     = ( $tp_pack->id === $current_theme_package_id ) ? 'active' : 'inactive';
					$required  = array( 'wp_required_version' => '', 'bp_required_version' => '' );
					$versions  = array_intersect_key( (array) $tp_pack, $required );
					$supported = self::check_versions( $versions ); ?>
					...
					<tr id="<?php echo esc_attr( $tp_pack->id ); ?>" class="<?php echo esc_attr( $tp_pack->id ) . ' ' . esc_attr( $class ); ?>">
						<th scope="row" class="check-column"><?php
							...
							if ( true === $supported ) :
								...
								?><input type="radio" id="bp_tpl_pack-<?php echo esc_attr( $tp_pack->id ); ?>" name="bp_tpl_pack[dir]" value="<?php echo esc_attr( $tp_pack->dir );?>"<?php checked( $tp_pack->id === $current_theme_package_id ); ?> /><?php
						...
						<td class="plugin-title" style="width: 190px;">
							...
							<label for="bp_tpl_pack-<?php echo esc_attr( $tp_pack->id ); ?>">
								<strong><?php echo esc_html( $tp_pack->name ); ?></strong>
							</label>
						...
						<td class="column-description desc">
							<div class="plugin-description">
								<p><?php echo esc_html( $tp_pack->description ); ?></p>
							</div>
		...
		if ( ! empty( $warnings ) ) :
			...
			?><div id="message" class="error"><?php
				...
				foreach ( $warnings as $warning ) :
					...
					?><p><?php echo esc_html( $warning ); ?></p><?php
